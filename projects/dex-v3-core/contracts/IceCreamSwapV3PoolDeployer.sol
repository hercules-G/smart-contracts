...
import "./IceCreamSwapV3Pool.sol";

contract IceCreamSwapV3PoolDeployer is IPancakeV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    function deploy(
        ...
        pool = address(new IceCreamSwapV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
